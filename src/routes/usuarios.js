...
router.post("/registrarPasoUno", async function (req, res) {
  var response = new ResponseResult();
  try {
    // response = await UsuarioCtrl.registroVerificarCarnet('12345678');
...
router.post("/registrarPasoDos", async function (req, res) {
  ...
  try {
    ...
    //   const fs = require("fs");
    //   const img = fs.readFileSync('i9.png')
    // response = await UsuarioCtrl.registroVerificarFoto("9020704", img);
  } catch (e) {
    response.ok = false;
    response.msg = "Excepcion al iniciar sesion: " + e;
  }
...
router.post("/registrarPasoTres", async function (req, res) {
  ...
  try {
    ...
    // response = await UsuarioCtrl.registroVerificarCorreo('holmoscampos@gmail.com');
  } catch (e) {
    response.ok = false;
    response.msg = "Excepcion al iniciar sesion: " + e;
  }
